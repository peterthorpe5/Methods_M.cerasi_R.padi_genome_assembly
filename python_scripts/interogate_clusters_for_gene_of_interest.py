...
if __name__ == '__main__':
    ...
    Total_elements_counter = Counter({'Mpe':0, 'Mca':0, 'Api': 0,
                                   'Rpa':0, 'Dno':0})
    Total_elements_matching_wanted_counter = Counter({'Mpe':0,
                                                      'Mca':0,
                                                      'Api': 0,
                                                      'Rpa':0,
                                                      'Dno':0})
    for line in cluster_data:
        cluster_with_other_species_count = 0

        line = line.rstrip()
        unique_counter_counter = Counter({'Mpe':0, 'Mca':0,
                                          'Api': 0,
                                         'Rpa':0, 'Dno':0})
        species_counter = Counter({'Mpe':0, 'Mca':0,
                                   'Api': 0,
                                   'Rpa':0, 'Dno':0})
        ...
        for gene in cluster_elements:
            ...
            if gene in wanted:
                ...
                if not line in clusters_of_interest:
                    ...
                    # count through th cluster again to see what
                    # speices are there
                    for gene in cluster_elements:
                        gene = gene.rstrip()
                        prefix = gene[:3]
                        if prefix in allowed:
                            # double check only allowed species are
                            # counted
                    ...
                    unique_counter_counter_od = collections.OrderedDict(sorted(unique_counter_counter.items()))
                    species_counter_str = ""
                    for key, vals in species_counter_od.items():
                        result = "%s\t%s\t" % (key, vals)
                        species_counter_str = species_counter_str + result
                        for spe in allowed:
                            if spe == wanted_prefix:
                                continue
                            found = species_counter_od[spe]
                            cluster_with_other_species_count = cluster_with_other_species_count + found
                            
                    unique_counter_counter_str = ""
                    for key, vals in unique_counter_counter_od.items():
                        result = "%s\t%s\t" % (key, vals)
                        unique_counter_counter_str = unique_counter_counter_str + result
                    if cluster_with_other_species_count < 2:
                        print line
